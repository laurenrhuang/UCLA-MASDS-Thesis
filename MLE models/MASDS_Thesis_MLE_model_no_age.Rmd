---
title: "MASDS Thesis - MLE Model (no age)"
author: "Lauren Huang"
date: "`r Sys.Date()`"
output: pdf_document
---
...
## MLE NO AGE, 6 Excitation, 3 Decay parameters DAILY
...
# # list of communities
communities <- list(
  "Santa Monica" = list(event_times = adj_event_times_santa_monica),
  "Silver Lake" = list(event_times = adj_event_times_silverlake),
  "Pomona" = list(event_times = adj_event_times_pomona),
  "Glendale" = list(event_times = adj_event_times_glendale),
  "El Monte" = list(event_times = adj_event_times_el_monte),
  "Little Armenia" = list(event_times = adj_event_times_little_armenia),
  "Signal Hill" = list(event_times = adj_event_times_signal_hill)
)


# intensity function
custom_intensity_mle <- function(params, t, event_times) {
  mu <- max(params[1], 1e-5)
  alpha <- params[2:7]
  beta <- params[8:10]
...
log_likelihood_mle <- function(params, event_times) {
  log_lik <- 0
  for (i in seq_along(event_times)) {
    t <- event_times[i]
    lambda_t <- custom_intensity_mle(params, t, event_times[1:(i-1)])
...
save_mle_results <- function(results, community, filename = "result_community_mle_6_3_no_age_daily_old.txt") {
...
# if you want to clear the saved parameter estimates
# if (file.exists("result_community_mle_6_3_no_age_daily_old.txt")) file.remove("result_community_mle_6_3_no_age_daily_old.txt")
# 
...
#     method = "L-BFGS-B",
#     lower = c(1e-5, rep(1e-5, 6), rep(1e-3, 3)),
#     upper = c(Inf, rep(10, 6), rep(10, 3)),
#     hessian = TRUE,
#     control = list(trace = 1, maxit = 300)
#   )
# 
#   # prepare results
#   mle_daily_results <- list(
#     baseline = fit$par[1],
#     excitation_params = fit$par[2:7],
#     decay_params = fit$par[8:10],
#     loglik = -fit$value,
...
# 
#   # save results
#   save_mle_results(mle_daily_results, community_name)
# 
#   # summary
...
read_mle_results <- function(filename = "result_community_mle_6_3_no_age_daily_old.txt") {
...
mle_daily_results <- read_mle_results()

# get all community names
community_names <- names(mle_daily_results)

# create a data frame of all baselines
baseline_df <- data.frame(
  Community = community_names,
  Baseline = sapply(mle_daily_results, function(x) x$baseline)
)

# create a matrix of all excitation parameters
excitation_matrix <- t(sapply(mle_daily_results, function(x) x$excitation_params))
colnames(excitation_matrix) <- paste0("Excitation_", 1:6)

# combine all parameters into one data frame
full_mle_daily_results <- do.call(rbind, lapply(names(mle_daily_results), function(comm) {
  data.frame(
    Community = comm,
    Baseline = mle_daily_results[[comm]]$baseline,
    Excitation = I(list(mle_daily_results[[comm]]$excitation_params)),
    Decay = I(list(mle_daily_results[[comm]]$decay_params)),
    LogLik = mle_daily_results[[comm]]$loglik
  )
}))

print(baseline_df)
print(excitation_matrix)
head(full_mle_daily_results)
```

```{r}
# list of all communities
community_data_mle <- list(
  "Santa Monica" = list(
    df = santa_monica,
...
process_and_plot_all <- function(community_name, data, adj_event_times, estimates) {
  ...
  for (i in seq_along(numeric_event_times)) {
    ...
    # calculate intensity
    expected_deaths[i] <- custom_intensity_mle(
      params = estimates,
      t = t,
...
generate_all_plots <- function(results_list, community_data) {
  ...
  for (comm in names(community_data_mle)) {
    ...
    all_plots[[comm]] <- process_and_plot_all(
      community_name = comm,
      data = community_data_mle[[comm]]$df,
      adj_event_times = community_data_mle[[comm]]$adj_times,
      estimates = c(
...
mle_daily_results <- read_mle_results("result_community_mle_6_3_no_age_daily_old.txt")

# generate plots
all_mle_results_daily <- generate_all_plots(mle_daily_results, community_data_mle)

# combine plots
(all_mle_results_daily$`Santa Monica`$plot | all_mle_results_daily$`Silver Lake`$plot | all_mle_results_daily$Pomona$plot) /
  (all_mle_results_daily$Glendale$plot | all_mle_results_daily$`El Monte`$plot |all_mle_results_daily$`Little Armenia`$plot) /
  all_mle_results_daily$`Signal Hill`$plot

# Santa Monica
all_mle_results_daily$`Santa Monica`$plot

# other communities
(all_mle_results_daily$`Silver Lake`$plot | all_mle_results_daily$Pomona$plot | all_mle_results_daily$Glendale$plot) /
  (all_mle_results_daily$`El Monte`$plot | all_mle_results_daily$`Little Armenia`$plot | all_mle_results_daily$`Signal Hill`$plot)
```
...
community_metrics <- lapply(names(all_mle_results_daily), function(comm) {
  df <- all_mle_results_daily[[comm]]$data
...
# log-likelihood function
log_likelihood <- function(p, community_name) {
    mu <- p[1]
    ...
    for (i in 1:days) {
        ...
        expectedn <- mu + K[ind] * sum(g * tmp)
        
        if (expectedn <= 0) {
          expectedn <- 1e-10  # very small value to avoid log(0)
        }
...
fit_daily_mle_6_3 <- function(community_name) {
    ...
    k <<- 1
    drw <- 0
    initial_params <- c(1, rep(0.5, 6), rep(1/4, 3))
...
result_mle_daily_6_3 <- matrix(0, ncol = 10, nrow = 7)

# clear file to start fresh before appending new results
# sink("result_community_mle_6_3_no_age_daily.txt")
# sink()
# for (m in 1:7) {
#     result_mle_daily_6_3[m,] = fit_daily_mle_6_3(communities[m])
#     # append to file
#     sink("result_community_mle_6_3_no_age_daily.txt", append=T)
#     cat(m, " ", result_mle_daily_6_3[m,], "\n")
#     sink()
# }

x1 = scan("C:/Users/cupca/Desktop/UCLA/Thesis/result_community_mle_6_3_no_age_daily.txt")
x2 <- matrix(x1, ncol = 11, nrow = 7, byrow = TRUE)
result_mle_daily_6_3 <- x2[1:7, 2:11]
```

```{r, warning = F}
# calculates expected deaths
compute_expected_deaths_mle_6_3 <- function(params, df, community_name) {
    mu <- params[1] 
    ...
    for (i in 1:days) {
        ...
        expected_deaths[i] <- mu + K[ind] * sum(g * tmp)
    }
...
```{r}
mle_daily_plot_sm <- ggplot(santa_monica, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], santa_monica,
...
mle_daily_plot_s <- ggplot(silverlake, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], silverlake,
...
mle_daily_plot_p <- ggplot(pomona, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], pomona,
...
mle_daily_plot_g <- ggplot(glendale, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], glendale,
...
mle_daily_plot_em <- ggplot(el_monte, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], el_monte,
...
mle_daily_plot_la <- ggplot(little_armenia, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], little_armenia,
...
mle_daily_plot_sh <- ggplot(signal_hill, aes(x = date)) +
  geom_line(aes(y = incidence, color = "Observed"), linewidth = 1) +
  geom_line(aes(y = compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], signal_hill,
...
# put plots together as one graphic
(mle_daily_plot_sm | mle_daily_plot_s | mle_daily_plot_p) / 
  (mle_daily_plot_g | mle_daily_plot_em | mle_daily_plot_la) / 
  (mle_daily_plot_sh)

mle_daily_plot_sm

(mle_daily_plot_s | mle_daily_plot_p | mle_daily_plot_g) /
  (mle_daily_plot_em | mle_daily_plot_la | mle_daily_plot_sh)
```
...
rmse_mle_daily_values <- numeric(7)
for (i in 1:7) {
    expected_deaths <- compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], get(tolower(communities[i])),
                                               communities[i])
    observed_deaths <- get(tolower(communities[i]))$incidence
    rmse_mle_daily_values[i] <- compute_rmse(observed_deaths, expected_deaths)
}

data.frame(Community = communities, RMSE = rmse_mle_daily_values)

# calculate MAE
compute_mae <- function(observed, expected) {
  mean(abs(observed - expected), na.rm = TRUE)
}
mae_mle_daily_values <- numeric(7)
for (i in 1:7) {
    expected_deaths <- compute_expected_deaths_mle_6_3(result_mle_daily_6_3[1, ], get(tolower(communities[i])),
                                               communities[i])
    observed_deaths <- get(tolower(communities[i]))$incidence
    mae_mle_daily_values[i] <- compute_mae(observed_deaths, expected_deaths)
}

data.frame(Community = communities, MAE = mae_mle_daily_values)
```
