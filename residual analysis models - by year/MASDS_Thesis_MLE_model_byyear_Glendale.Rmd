---
title: "MASDS Thesis - MLE model - by year Glendale"
author: "Lauren Huang"
date: "`r Sys.Date()`"
output: pdf_document
---
...
# split Glendale dates by year
glendale_by_year <- glendale %>%
  mutate(year = format(date, "%Y"))

glendale_2020 <- filter(glendale_by_year, year == "2020")
glendale_2021 <- filter(glendale_by_year, year == "2021")
glendale_2022 <- filter(glendale_by_year, year == "2022")

# convert dates to numeric time (days since the start of the data)
start_date_2020 <- min(glendale_2020$date, na.rm = TRUE)
glendale_2020 <- glendale_2020 %>% mutate(time = as.numeric(difftime(date, start_date_2020, units = "days")))

start_date_2021 <- min(glendale_2021$date, na.rm = TRUE)
glendale_2021 <- glendale_2021 %>% mutate(time = as.numeric(difftime(date, start_date_2021, units = "days")))

start_date_2022 <- min(glendale_2022$date, na.rm = TRUE)
glendale_2022 <- glendale_2022 %>% mutate(time = as.numeric(difftime(date, start_date_2022, units = "days")))
...
event_times_glendale_2020 <- glendale_2020 %>%
  group_by(name) %>%
  summarise(event_times = list(unlist(mapply(generate_event_times, time, incidence))))

event_times_glendale_2020 <- event_times_glendale_2020 %>% pull(event_times)
event_times_glendale_2020 <- sort(unlist(event_times_glendale_2020))
event_times_glendale_2020_unique <- unique(event_times_glendale_2020)
adj_event_times_glendale_2020 <- event_times_glendale_2020_unique + 
  seq(0, length(event_times_glendale_2020_unique) - 1) * 1e-5

# generate event times for 2021
event_times_glendale_2021 <- glendale_2021 %>%
  group_by(name) %>%
  summarise(event_times = list(unlist(mapply(generate_event_times, time, incidence))))

event_times_glendale_2021 <- event_times_glendale_2021 %>% pull(event_times)
event_times_glendale_2021 <- sort(unlist(event_times_glendale_2021))
event_times_glendale_2021_unique <- unique(event_times_glendale_2021)
adj_event_times_glendale_2021 <- event_times_glendale_2021_unique + 
  seq(0, length(event_times_glendale_2021_unique) - 1) * 1e-5

# generate event times for 2022
event_times_glendale_2022 <- glendale_2022 %>%
  group_by(name) %>%
  summarise(event_times = list(unlist(mapply(generate_event_times, time, incidence))))

event_times_glendale_2022 <- event_times_glendale_2022 %>% pull(event_times)
event_times_glendale_2022 <- sort(unlist(event_times_glendale_2022))
event_times_glendale_2022_unique <- unique(event_times_glendale_2022)
adj_event_times_glendale_2022 <- event_times_glendale_2022_unique + 
  seq(0, length(event_times_glendale_2022_unique) - 1) * 1e-5
```
...
event_times_list <- list(
  "2020" = adj_event_times_glendale_2020,
  "2021" = adj_event_times_glendale_2021,
  "2022" = adj_event_times_glendale_2022
)

age_dist_g <- age_data_g[-9] / sum(age_data_g[-9])
```
...
save_glendale_results <- function(results, community, filename = "result_community_mle_6_3_age_daily.txt") {
...
# if (file.exists("result_glendale_old.txt")) file.remove("result_glendale_old.txt")
# 
# # loops through years and estimates parameters
# for (year in years) {
#   cat("\n=== Processing Glendale", year, "===\n")
# 
...
# 
#   # save results
#   save_glendale_results(mle_age_results_daily, paste0("Glendale_", year),
#                              filename = "result_glendale_old.txt")
# 
#   # summary
#   cat("\nCompleted Glendale", year, "with convergence", fit$convergence, "\n")
...
read_glendale_results <- function(filename = "result_glendale_old.txt") {
...
glendale_results <- read_glendale_results()

community_names <- names(glendale_results)

# create a data frame of all baselines
baseline_df <- data.frame(
  Community = community_names,
  Baseline = sapply(glendale_results, function(x) x$baseline)
)

# create a matrix of all excitation parameters
excitation_matrix <- t(sapply(glendale_results, function(x) x$excitation_params))
colnames(excitation_matrix) <- paste0("Excitation_", 1:6)

# combine all parameters into one data frame
full_glendale_results <- do.call(rbind, lapply(names(glendale_results), function(comm) {
  data.frame(
    Community = comm,
    Baseline = glendale_results[[comm]]$baseline,
    Age_Coeffs = I(list(glendale_results[[comm]]$age_coefficients)),
    Excitation = I(list(glendale_results[[comm]]$excitation_params)),
    Decay = I(list(glendale_results[[comm]]$decay_params)),
    LogLik = glendale_results[[comm]]$loglik
  )
}))

print(baseline_df)
print(excitation_matrix)
head(full_glendale_results)
```
...
glendale_data <- list(
  "Glendale_2020" = list(
    df = glendale_2020,
    adj_times = adj_event_times_glendale_2020,
    age_data = age_data_g
  ),
  "Glendale_2021" = list(
    df = glendale_2021,
    adj_times = adj_event_times_glendale_2021,
    age_data = age_data_g
  ),
  "Glendale_2022" = list(
    df = glendale_2022,
    adj_times = adj_event_times_glendale_2022,
    age_data = age_data_g
  )
)
...
glendale_results <- read_glendale_results("result_glendale_old.txt")

# plot
all_glendale_plots <- generate_all_plots(glendale_results, glendale_data)

# plot by year
all_glendale_plots$Glendale_2020$plot
all_glendale_plots$Glendale_2021$plot
all_glendale_plots$Glendale_2022$plot
...
glendale_metrics <- lapply(names(all_glendale_plots), function(year_key) {
  df <- all_glendale_plots[[year_key]]$data
...
metrics_glendale_1 <- do.call(rbind, glendale_metrics)
metrics_glendale_1
```
...
for (year_key in names(all_glendale_plots)) {
  df_year <- all_glendale_plots[[year_key]]$data
  residuals_df <- data.frame(
    date = df_year$date,
    year = as.numeric(gsub("Glendale_", "", year_key)), # get the year number
...
residuals_plot_1 <- ggplot(all_residuals_df_1, aes(x = date, y = residual)) +
  ...
  labs(title = "Residuals in Glendale by year (all past data)",
       x = "Date",
       y = "Residuals") +
  theme_minimal()
...
for (year_key in names(all_glendale_plots)) {
  ...
  residuals_df <- data.frame(
    date = df_year$date,
    year = as.numeric(gsub("Glendale_", "", year_key)),
    observed = df_year$observed,
    expected = df_year$expected,
    residual = df_year$observed - df_year$expected,
...
std_residuals_plot_1 <- ggplot(all_residuals_df_1, aes(x = date, y = std_residual)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.8) +
  facet_wrap(~ year, scales = "free_x", ncol = 2) +
  labs(title = "Standardized Residuals in Glendale by Year (all past data)",
       x = "Date",
...
```{r}
params_2020 <- c(
  glendale_results[["Glendale_2020"]]$baseline,
  glendale_results[["Glendale_2020"]]$age_coefficients,
  glendale_results[["Glendale_2020"]]$excitation_params,
  glendale_results[["Glendale_2020"]]$decay_params
)

# calculate Ogata rescaled times
z_vals_2020 <- compute_rescaled_times_1(
  adj_event_times_glendale_2020,
  params_2020,
  age_dist_g
)

# histogram
hist(z_vals_2020, breaks = 20, main = "Glendale 2020 Ogata rescaled inter-event times", 
     xlab = "z", freq = FALSE)
curve(dexp(x, rate = 1), add = TRUE, col = "blue", lwd = 2)

# QQ plot
qqplot(qexp(ppoints(length(z_vals_2020))), z_vals_2020,
       main = "Glendale 2020 QQ plot (Exp(1))",
...
```{r}
params_2021 <- c(
  glendale_results[["Glendale_2021"]]$baseline,
  glendale_results[["Glendale_2021"]]$age_coefficients,
  glendale_results[["Glendale_2021"]]$excitation_params,
  glendale_results[["Glendale_2021"]]$decay_params
)

# calculate Ogata rescaled times
z_vals_2021 <- compute_rescaled_times_1(
  adj_event_times_glendale_2021,
  params_2021,
  age_dist_g
)

# histogram
hist(z_vals_2021, breaks = 20, main = "Glendale 2021 Ogata rescaled inter-event times", 
     xlab = "z", freq = FALSE)
curve(dexp(x, rate = 1), add = TRUE, col = "blue", lwd = 2)

# QQ plot
qqplot(qexp(ppoints(length(z_vals_2021))), z_vals_2021,
       main = "Glendale 2021 QQ plot (Exp(1))",
       xlab = "theoretical quantiles", ylab = "observed quantiles")
abline(0, 1, col = "red")
...
```{r}
params_2022 <- c(
  glendale_results[["Glendale_2022"]]$baseline,
  glendale_results[["Glendale_2022"]]$age_coefficients,
  glendale_results[["Glendale_2022"]]$excitation_params,
  glendale_results[["Glendale_2022"]]$decay_params
)

# calculate Ogata rescaled times
z_vals_2022 <- compute_rescaled_times_1(
  adj_event_times_glendale_2022,
  params_2022,
  age_dist_g
)

# histogram
hist(z_vals_2022, breaks = 20, main = "Glendale 2022 Ogata rescaled inter-event times", 
     xlab = "z", freq = FALSE)
curve(dexp(x, rate = 1), add = TRUE, col = "blue", lwd = 2)

# QQ plot
qqplot(qexp(ppoints(length(z_vals_2022))), z_vals_2022,
       main = "Glendale 2022 QQ plot (Exp(1))",
       xlab = "theoretical quantiles", ylab = "observed quantiles")
abline(0, 1, col = "red")
...
ggplot(median_residuals_df_1, aes(x = median_residual)) +
  ...
  labs(title = "Histogram of median residuals Glendale by year (all past data)",
...
hist(std_res_2020, breaks = 20, probability = TRUE,
     main = "Glendale 2020 Standardized Residuals",
     xlab = "standardized residuals")
curve(dnorm(x, mean = 0, sd = 1), col = "blue", lwd = 2, add = TRUE)

qqnorm(std_res_2020, main = "Glendale 2020 QQ Plot Standardized Residuals (Normal)")
...
hist(std_res_2021, breaks = 20, probability = TRUE,
     main = "Glendale 2021 Standardized Residuals",
     xlab = "standardized residuals")
curve(dnorm(x, mean = 0, sd = 1), col = "blue", lwd = 2, add = TRUE)

qqnorm(std_res_2021, main = "Glendale 2021 QQ Plot Standardized Residuals (Normal)")
...
hist(std_res_2022, breaks = 20, probability = TRUE,
     main = "Glendale 2022 Standardized Residuals",
     xlab = "standardized residuals")
curve(dnorm(x, mean = 0, sd = 1), col = "blue", lwd = 2, add = TRUE)

qqnorm(std_res_2022, main = "Glendale 2022 QQ Plot Standardized Residuals (Normal)")
...
log_likelihood <- function(p, year, lambda = 0.01) {
    ...
    # age distribution
    age_data <- get_age_distribution("Glendale")[1:8]
    ...
    for (i in 1:days) {
        ...
        # Ensure tmp and age_weights have compatible lengths
        tmp <- rev(tmp)
...
fit_glendale_by_year <- function(year) {
    df_name <- paste0("glendale_", year)
...
result_glendale <- matrix(0, ncol = 18, nrow = 3)

# clear file to start fresh before appending new results
# sink("result_glendale.txt")
# sink()
# for (m in 1:3) {
#     result_glendale[m,] = fit_glendale_by_year(years[m])
#     # append to file
#     sink("result_glendale.txt", append=T)
#     cat(m, " ", result_glendale[m,], "\n")
#     sink()
# }

x1 = scan("C:/Users/cupca/Desktop/UCLA/Thesis/result_glendale.txt") 
x2 <- matrix(x1, ncol = 19, nrow = 3, byrow = TRUE)
result_glendale <- x2[1:3, 2:19]
result_glendale_coefficients <- result_glendale[, 11:18]
```

```{r, warning = F}
# function for calculating expected deaths
compute_expected_deaths_glendale <- function(params, df, community_name) {
    mu <- params[1]
...
for (m in 1:3) {
    ...
    df_name <- paste0("glendale_", year)
    df <- get(df_name, envir = .GlobalEnv)
    params <- result_glendale[m, ]
    expected_deaths <- compute_expected_deaths_glendale(params, df, 
                                                        community_name = "Glendale")
...
for (m in 1:3) {
    ...
    expected_deaths <- compute_expected_deaths_glendale(params, df, community_name = "Glendale")
    ...
    p <- ggplot(df, aes(x = date)) +
      ...
      geom_line(aes(y = expected, color = "Expected"), linewidth = 1) +
      scale_color_manual(values = c("red", "black")) +
      labs(title = paste("Expected vs. Observed Deaths for Glendale", year),
...
for (m in seq_along(years)) {
  year <- years[m]
  df <- get(paste0("glendale_", year))
  
  expected_deaths <- compute_expected_deaths_glendale(result_glendale[m, ], df, "Glendale")
...
metrics_glendale_2 <- do.call(rbind, metrics_list)
metrics_glendale_2
```
...
for (y in 1:length(year_dfs)) {
    ...
    df_year <- year_dfs[[y]]
    expected_deaths <- compute_expected_deaths_glendale(result_glendale[y, ], 
                                                            df_year, "Glendale")
...
residuals_plot_2 <- ggplot(all_residuals_df_2, aes(x = date, y = residual)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.8) +
  facet_wrap(~ year, scales = "free_x", ncol = 2) +
  labs(title = "Residuals in Glendale by year (16 day window approach)",
       x = "Date",
       y = "Residuals") +
  theme_minimal()
...
std_residuals_plot_2 <- ggplot(all_residuals_df_2, aes(x = date, y = std_residual)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.8) +
  facet_wrap(~ year, scales = "free_x", ncol = 2) +
  labs(title = "Standardized Residuals in Glendale by Year (16-day rolling window)",
       x = "Date",
       y = "Standardized Residuals") +
  theme_minimal()
...
```{r}
# compute Ogata rescaled inter-event times
compute_rescaled_times_2 <- function(df, params, community_name = "Glendale") {
  mu <- params[1] 
  K <- params[2:7]
...
```{r}
df_2020 <- glendale_2020
params_2020 <- result_glendale[1, ]
...
hist(z_vals_2020, breaks = 20, probability = TRUE, 
     main = "Glendale 2020 Ogata rescaled inter-event times",
...
```{r}
df_2021 <- glendale_2021
params_2021 <- result_glendale[1, ]
...
hist(z_vals_2021, breaks = 20, probability = TRUE, 
     main = "Glendale 2021 Ogata rescaled inter-event times",
     xlab = "z (rescaled time)")
curve(dexp(x, rate = 1), add = TRUE, col = "blue", lwd = 2)
...
```{r}
df_2022 <- glendale_2022
params_2022 <- result_glendale[1, ]
...
hist(z_vals_2022, breaks = 20, probability = TRUE, 
     main = "Glendale 2022 Ogata rescaled inter-event times",
     xlab = "z (rescaled time)")
curve(dexp(x, rate = 1), add = TRUE, col = "blue", lwd = 2)
...
ggplot(median_residuals_df_2, aes(x = median_residual)) +
  geom_histogram(bins = 30, fill = "lightblue", color = "black") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red", linewidth = 0.8) +
  labs(title = "Histogram of median residuals Glendale by year (16-day rolling window)",
       x = "median daily residual",
       y = "frequency") +
  facet_wrap(~ year, ncol = 2) +
  theme_minimal()
...
qqnorm(std_res_2020,
       main = "Glendale 2020 QQ Plot Standardized Residuals (Normal)")
qqline(std_res_2020, col = "red")
...
qqnorm(std_res_2021,
       main = "Glendale 2021 QQ Plot Standardized Residuals (Normal)")
qqline(std_res_2021, col = "red")
...
qqnorm(std_res_2022,
       main = "Glendale 2022 QQ Plot Standardized Residuals (Normal)")
qqline(std_res_2020, col = "red")
