---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Little Armenia"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
little_armenia <- community_data %>% filter(name == "Los Angeles - Little Armenia")
dd.data <- little_armenia %>%
  filter(incidence > 0) %>%
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_la / sum(age_data_la)
```
...
lambda_ <- function(th, t, ti.v, segment_info, age_data) {
  ...
  if (length(ti.v) == 0) return(mu)  # If no events in the window, return background intensity
...
th.init <- list(
  th.mu = 0,
  th.K1 = 0, th.K2 = 0, th.K3 = 0, th.K4 = 0, th.K5 = 0, th.K6 = 0,
  th.c1 = 0, th.c2 = 0, th.c3 = 0,
...
covid_fit_age_multi <- Hawkes.bru(
  ...
  age_data = age_data_la[-9]
)

summary(covid_fit_age_multi) # converged at 6 iterations
```
...
# Little Armenia dates
time_grid <- little_armenia$date
event_times <- little_armenia$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_la[-9])
})

# store total expected deaths for making map
total_expected_deaths_la <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = little_armenia$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_little_armenia_new <- ggplot(comparison_df, aes(x = date)) +
  ...
  labs(title = "Expected vs Observed COVID Deaths: Little Armenia",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_little_armenia_new
```
...
metrics_table <- data.frame(Community = "Little Armenia",
