---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Glendale"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
glendale <- community_data %>% filter(name == "City of Glendale")
dd.data <- glendale %>%
  filter(incidence > 0) %>%
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_g / sum(age_data_g)
```
...
params <- list(
  mu = link.f.be$mu(post_means[1]),  # First element is th.mu
  # excitation params
  K_vec = c(
...
# Glendale dates
time_grid <- glendale$date
event_times <- glendale$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_g[-9])
})

# store total expected deaths for making map
total_expected_deaths_g <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = glendale$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_glendale_new <- ggplot(comparison_df, aes(x = date)) +
  ...
  labs(title = "Expected vs Observed COVID Deaths: Glendale",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_glendale_new
```
...
metrics_table <- data.frame(Community = "Glendale",
