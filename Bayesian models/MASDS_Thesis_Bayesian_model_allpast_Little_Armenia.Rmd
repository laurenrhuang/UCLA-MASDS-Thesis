---
title: "MASDS Thesis - Bayesian model all past data - Little Armenia"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
# convert to dates
first_date <- min(little_armenia$date)
K_break_dates <- first_date + K_break_days
c_break_dates <- first_date + c_break_days

segment_info <- data.frame(
  break_dates = c(K_break_dates, max(little_armenia$date) + 1),
  K_idx = 1:7,
...
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], segment_info, age_data_la[-9])
})
...
bayes_6_3_little_armenia_plot <- ggplot(comparison_df, aes(x = date)) +
