---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Pomona"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
pomona <- community_data %>% filter(name == "City of Pomona")
dd.data <- pomona %>%
  filter(incidence > 0) %>%
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_p / sum(age_data_p)
```
...
# Pomona dates
time_grid <- pomona$date
event_times <- pomona$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_p[-9])
})

# store total expected deaths for making map
total_expected_deaths_p <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = pomona$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_pomona_new <- ggplot(comparison_df, aes(x = date)) +
  ...
  labs(title = "Expected vs Observed COVID Deaths: Pomona",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_pomona_new
```
...
metrics_table <- data.frame(Community = "Pomona",
