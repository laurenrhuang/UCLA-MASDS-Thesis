---
title: "MASDS Thesis - Bayesian model all past data - Signal Hill"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
# link.f.be <- list(
#   mu = \(x) loggaus.t(x, log(0.5), 1),
#   K1 = \(x) lognorm.t(x, log(0.5), 0.4),
#   K2 = \(x) lognorm.t(x, log(0.5), 0.4),
#   K3 = \(x) lognorm.t(x, log(0.5), 0.4),
#   K4 = \(x) lognorm.t(x, log(0.5), 0.4),
#   K5 = \(x) lognorm.t(x, log(0.5), 0.4),
#   K6 = \(x) lognorm.t(x, log(0.5), 0.4),
#   c1 = \(x) loggaus.t(x, log(3), 0.5),
#   c2 = \(x) loggaus.t(x, log(3), 0.5),
#   c3 = \(x) loggaus.t(x, log(3), 0.5),
#   alpha_age1 = \(x) unif.t(x, -2, 2),
#   alpha_age2 = \(x) unif.t(x, -1, 1),
#   alpha_age3 = \(x) unif.t(x, -1, 1),
#   alpha_age4 = \(x) unif.t(x, -1, 1),
...
# convert to dates
first_date <- min(signal_hill$date)
K_break_dates <- first_date + K_break_days
c_break_dates <- first_date + c_break_days

segment_info <- data.frame(
  break_dates = c(K_break_dates, max(signal_hill$date) + 1),
  K_idx = 1:7,
...
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], segment_info, age_data_sh[-9])
})
...
bayes_6_3_signal_hill_plot <- ggplot(comparison_df, aes(x = date)) +
...
(bayes_6_3_silverlake_plot | bayes_6_3_pomona_plot | bayes_6_3_glendale_plot) /
(bayes_6_3_el_monte_plot | bayes_6_3_little_armenia_plot | bayes_6_3_signal_hill_plot)
```
...
# total_expected_deaths_sm <- 677.1748
# total_expected_deaths_s <- 655.6438
# total_expected_deaths_p <- 763.3264
# total_expected_deaths_g <- 853.1658
# total_expected_deaths_em <- 761.2477
# total_expected_deaths_la <- 626.4327
# total_expected_deaths_sh <- 621.3315
...
ggplot(geodata_bayes_zoom) +
  ...
  labs(title = "All past data age Bayesian model estimated COVID-19 deaths",
