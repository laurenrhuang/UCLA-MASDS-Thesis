---
title: "MASDS Thesis - Bayesian model all past data - Glendale"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
summary(covid_fit_age_multi) # converged at 17 iterations
```
...
# convert to dates
first_date <- min(glendale$date)
K_break_dates <- first_date + K_break_days
c_break_dates <- first_date + c_break_days

segment_info <- data.frame(
  break_dates = c(K_break_dates, max(glendale$date) + 1),
  K_idx = 1:7,
...
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], segment_info, age_data_g[-9])
})
...
bayes_6_3_glendale_plot <- ggplot(comparison_df, aes(x = date)) +
