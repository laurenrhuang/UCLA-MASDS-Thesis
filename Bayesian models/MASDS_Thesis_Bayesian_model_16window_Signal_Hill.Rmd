---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Signal Hill"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
```{r}
# adjusting to have each death incidence be its own row
signal_hill <- community_data %>% filter(name == "City of Signal Hill")
dd.data <- signal_hill %>%
  filter(incidence > 0) %>%
  uncount(incidence, .id = "death_num") %>%
  mutate(
    time_date = as.POSIXct(paste0(date)),
    # small random offset to prevent same times
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_sh / sum(age_data_sh)
```
...
lambda_ <- function(th, t, ti.v, segment_info, age_data) {
  ...
  # Keep only events in the last 16 days
  ti.v <- ti.v[ti.v <= t & ti.v > t - 16]
  
  if (length(ti.v) == 0) return(mu)
  ...
  triggered_intensity <- sum(K_values * exp(-(t - ti.v) / c_values))  # this line is equivalent to gt function
...
# assemble segment_info with one row per segment
segment_info <- data.frame(
  breaks = K_breaks[-length(K_breaks)],
  K_idx = K_idx,
  c_idx = c_idx
)
# final row to include the last Inf break
...
Hawkes.bru <- function(sample.s, T1, T2, link.functions = NULL, 
  ...
  # create time bins for each observation
...
covid_fit_age_multi <- Hawkes.bru(
  ...
  age_data = age_data_sh[-9]
)

summary(covid_fit_age_multi) # converged at 5 iterations
```
...
lambda_ <- function(th, t, ti.v, age_data) {
  age_prop <- age_data / sum(age_data)
  ...
  t_num <- as.numeric(t - start_date)  # time of intensity calculation
  ti.v_num <- as.numeric(ti.v - start_date)
  
  # filter event times within window of the last 16 days
  ti.v_within_window <- ti.v_num[ti.v_num >= (t_num - 16) & ti.v_num < t_num]
  
  if (length(ti.v_within_window) == 0) return(mu_adj)
  
  # convert event times back to dates for segment assignment
  event_dates <- start_date + ti.v_within_window
  
  # convert break dates to Date objects
  K_break_dates <- as.Date(c("2020-05-17", "2020-07-16", "2020-09-14", 
                             "2020-11-13", "2021-02-11", "2021-08-10"))
  c_break_dates <- as.Date(c("2020-05-17", "2020-11-13", "2021-05-12"))
  
  # map event dates to segment indices
  seg_info <- data.frame(
    K_idx = findInterval(event_dates, K_break_dates),
    c_idx = findInterval(event_dates, c_break_dates)
  )
  ...
  # calculate the triggered intensity using the time decay formula
  triggered_intensity <- sum(K_values * exp(-(t_num - ti.v_within_window) / c_values))
  
  # return total intensity = background rate + triggered intensity
  return(mu_adj + triggered_intensity)
}

# Signal Hill dates
time_grid <- signal_hill$date
event_times <- signal_hill$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_sh[-9])
})

# store total expected deaths for making map
total_expected_deaths_sh <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = signal_hill$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_signal_hill_new <- ggplot(comparison_df, aes(x = date)) +
  ...
  labs(title = "Expected vs Observed COVID Deaths: Signal Hill",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_signal_hill_new
```
...
metrics_table <- data.frame(Community = "Signal Hill",
...
(bayes_6_3_age_silverlake_new | bayes_6_3_age_pomona_new | bayes_6_3_age_glendale_new) /
(bayes_6_3_age_el_monte_new | bayes_6_3_age_little_armenia_new | bayes_6_3_age_signal_hill_new)
```

```{r}
library(sf)

# save total expected deaths for future use
# total_expected_deaths_sm <- 660.3057
# total_expected_deaths_s <- 640.2055
# total_expected_deaths_p <- 762.0866
# total_expected_deaths_g <- 813.0485
# total_expected_deaths_em <- 739.9087
# total_expected_deaths_la <- 622.3912
# total_expected_deaths_sh <- 621.2013
...
bayes_expected_df <- data.frame(
  ...
  bayes_expected_deaths = c(
    total_expected_deaths_sm,
    total_expected_deaths_s,
    total_expected_deaths_p,
    total_expected_deaths_g,
    total_expected_deaths_em,
    total_expected_deaths_la,
    total_expected_deaths_sh
  )
)

# join spatial data with expected total deaths
geodata_bayes <- geodata %>%
  left_join(bayes_expected_df, by = "name")

# zoom in
geodata_bayes_zoom <- geodata_bayes %>%
  filter(!is.na(bayes_expected_deaths))

# plot 
ggplot(geodata_bayes_zoom) +
  geom_sf(aes(fill = bayes_expected_deaths), color = "white") +
  geom_sf_text(aes(label = name), size = 2, color = "black") +
  scale_fill_viridis_c(option = "plasma", na.value = "grey80", name = "Expected Deaths") +
  labs(title = "16-day rolling window age Bayesian model estimated COVID-19 deaths",
