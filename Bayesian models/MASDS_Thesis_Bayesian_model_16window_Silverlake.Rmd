---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Silverlake"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
silverlake <- community_data %>% filter(name == "Los Angeles - Silverlake")
dd.data <- silverlake %>%
  filter(incidence > 0) %>% 
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_s / sum(age_data_s)
```
...
# Sample generation with age covariates
...
# Silverlake dates
time_grid <- silverlake$date
event_times <- silverlake$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_s[-9])
})

# store total expected deaths for making map
total_expected_deaths_s <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = silverlake$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_silverlake_new <- ggplot(comparison_df, aes(x = date)) +
  ...
  labs(title = "Expected vs Observed COVID Deaths: Silverlake",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_silverlake_new
```
...
metrics_table <- data.frame(Community = "Silverlake",
