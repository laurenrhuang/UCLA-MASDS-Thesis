---
title: "MASDS Thesis - Bayesian model 16-day rolling window - Santa Monica"
author: "Lauren Huang"
date: "`r Sys.Date()`"
...
santa_monica <- community_data %>% filter(name == "City of Santa Monica")
dd.data <- santa_monica %>%
  filter(incidence > 0) %>%
...
T1 = 0
T2 <- max(data.bru$ts)
age_prop <- age_data_sm / sum(age_data_sm)
```
...
Hawkes.bru <- function(sample.s, T1, T2, link.functions = NULL, 
  ...
  # prepare triggered event calculation
...
# Santa Monica dates
time_grid <- santa_monica$date
event_times <- santa_monica$date

# calculate expected intensity
expected_intensity <- sapply(time_grid, function(t) {
  lambda_(params, t, event_times[event_times < t], age_data_sm[-9])
})

# store total expected deaths for making map
total_expected_deaths_sm <- sum(expected_intensity)

comparison_df <- data.frame(
  date = time_grid,
  expected = expected_intensity,
  observed = santa_monica$incidence
)

# plot expected vs observed deaths
bayes_6_3_age_santa_monica_new <- ggplot(comparison_df, aes(x = date)) +
  geom_line(aes(y = observed, color = "Observed"), linewidth = 0.8) +
  geom_line(aes(y = expected, color = "Expected"), linewidth = 0.8) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") + # Better date formatting
  scale_color_manual(values = c("Observed" = "black", "Expected" = "red")) +
  labs(title = "Expected vs Observed COVID Deaths: Santa Monica",
       x = "Date", y = "Daily Deaths") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1),
        legend.position = "right")
bayes_6_3_age_santa_monica_new
```
...
metrics_table <- data.frame(Community = "Santa Monica",
